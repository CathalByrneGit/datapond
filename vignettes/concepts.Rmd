...
This vignette explains the core concepts behind `datapond` - what data lakes are, how DuckLake works, and how to use it effectively. No prior knowledge assumed.
...
For high-cardinality columns (like user_id, order_id), bucket partitioning distributes data into a fixed number of files using Iceberg-compatible Murmur3 hashing:
...
**When to use bucket partitioning:**
- High-cardinality columns (millions of distinct values)
- Hive partitioning would create too many small files
- You need efficient point lookups by the partitioned column
...
- Time-series queries (`WHERE event_date BETWEEN ...`)
- Range scans on clustered columns
- Queries that filter on multiple clustered columns

### 8. Data Inlining (DuckLake 1.0+)

Data inlining solves the "small file problem" for streaming/frequent writes by staging data in the catalog database instead of creating new parquet files:
...
- Frequent small writes (streaming data, real-time updates)
- IoT or event data arriving in small batches
- Avoiding many small parquet files

### 9. Iceberg Compatibility (DuckLake 1.0+)

Export DuckLake tables to Iceberg format for use with other engines (Spark, Trino, Presto):
...
For stricter schema control, you can specify exact column types instead of relying on automatic inference:
...
- Interoperability with other systems expecting specific types
- BIGINT for large IDs (R integers max out at ~2 billion)
...
**When to use Arrow integration:**
- Interoperability with Arrow-based tools (sparklyr, polars, reticulate)
- Large datasets where you want direct Parquet access
- Avoiding DuckDB compute overhead for simple transfers
- Working with Arrow-native data pipelines

### 12. Views and Macros

DuckLake stores SQL views and macros in the catalog, supporting time travel - attach at a previous snapshot and you'll see the view/macro definitions that existed at that point.
...
Macros are reusable SQL expressions - either scalar (return a value) or table-valued (return a table):
...
Add structured metadata to tables and columns using native SQL COMMENT (stored in catalog with time travel, persists across disconnect/reconnect):
...
Metadata is stored as JSON in DuckLake's native COMMENT ON, which survives disconnect/reconnect and supports time travel.
...
This works well for:
- Data from external sources (CSV, APIs, other databases)
- Small to medium datasets that fit in R memory
- Data that needs R-specific transformations
...
- No `collect()` needed - data never leaves DuckDB
- No R memory used - handles datasets larger than RAM
- Faster - single SQL operation instead of round-trip
- The dplyr pipeline is converted to `CREATE TABLE AS SELECT`

**When to use lazy tables:**
- In-lake transformations (raw → clean → analysis)
- Aggregations and summaries
- Filtering large tables to smaller subsets
- Joining tables within the lake
- Any operation where source and destination are both in DuckLake
...
Documentation is stored using DuckLake's native COMMENT ON with JSON for structured metadata. This survives disconnect/reconnect and supports time travel.
...
Track where your data comes from and how it was transformed. datapond supports two approaches: manual lineage, where you describe sources yourself, and automatic column-level lineage, which uses the optional dplyneage package to analyse the query tree without executing it.

### Manual Lineage

The simplest approach is to record lineage yourself when creating derived tables:
...
Lineage information is stored in the table's native COMMENT as JSON alongside other metadata.

### Automatic Column-Level Lineage

For richer lineage, pass a lazy dbplyr table to the `pipeline=` argument. datapond uses the dplyneage package to walk the query tree and automatically extract which source columns feed each output column — without executing the query.

```{r}
# Build your pipeline as a lazy dbplyr table (not yet executed)
pipeline <- db_read(schema = "raw", table = "transactions") |>
  left_join(db_read(schema = "raw", table = "products"), by = "product_id") |>
  group_by(month, category) |>
  summarise(revenue = sum(amount), units = n(), .groups = "drop")

# Record lineage — sources and column edges are inferred from the pipeline
db_lineage(
  schema = "analysis",
  table = "monthly_summary",
  pipeline = pipeline,
...
When `pipeline=` is supplied, `sources=` can be omitted; datapond infers the source tables from the query plan. You can still pass `sources=` explicitly if you want to override or supplement what is detected.

### Tracking Lineage at Write Time

You can record lineage automatically when writing a table by setting `track_lineage = TRUE` on `db_write()`:

```{r}
db_read(schema = "raw", table = "transactions") |>
  left_join(db_read(schema = "raw", table = "products"), by = "product_id") |>
  group_by(month, category) |>
  summarise(revenue = sum(amount), units = n(), .groups = "drop") |>
  db_write(
...
This is equivalent to calling `db_write()` followed by `db_lineage(pipeline = ...)` but in a single step.

### Retrieving Lineage

```{r}
db_get_lineage(schema = "analysis", table = "monthly_summary")
#> $sources
...
#> 1 raw.transactions         month      analysis         month
#> 2     raw.products      category      analysis      category
#> 3 raw.transactions        amount      analysis       revenue
#> 4 raw.transactions    product_id      analysis         units
```

When column-level lineage is available, `db_get_lineage()` returns a `column_edges` data frame with one row per source-to-destination column mapping. Tables recorded with manual lineage only return `NULL` for `column_edges`.

### Visualising Lineage

When column-level lineage has been recorded, `db_lineage_flow()` renders an interactive React Flow diagram in the RStudio Viewer or a browser:

```{r}
db_lineage_flow(schema = "analysis", table = "monthly_summary")
```

The diagram shows source tables on the left, the destination table on the right, and edges for each column mapping. Nodes are colour-coded by schema. This function requires dplyneage to be installed.

### Installing dplyneage

Column-level lineage features (`pipeline=`, `track_lineage = TRUE`, and `db_lineage_flow()`) depend on the optional dplyneage package. Manual lineage works without it.
...
| `db_vacuum()` | Monthly | Remove old snapshots |
