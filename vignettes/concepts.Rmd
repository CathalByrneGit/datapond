...
Add SQL COMMENT to tables and columns (stored in catalog with time travel):
```{r}
# Comment on table
...
This is distinct from `db_describe()` which stores richer metadata (owner, tags) in a separate metadata table.
...
Lineage information is stored in the `_metadata.lineage` table within your DuckLake catalog.
