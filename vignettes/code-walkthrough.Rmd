...
db_file_stats <- function(schema = NULL, table = NULL) {
  ...
  tables <- get_catalog_tables(catalog, schema, table)

  # For each table, query file stats using ducklake_list_files
  results <- lapply(tables, function(tbl) {
    files_sql <- glue::glue("
      SELECT COUNT(*) AS file_count,
             COALESCE(SUM(row_count), 0) AS total_rows,
             COALESCE(SUM(file_size_bytes), 0) AS total_bytes
      FROM ducklake_list_files({catalog}, {tbl$name}, schema => {tbl$schema})
      WHERE data_file IS NOT NULL
    ")
    DBI::dbGetQuery(con, files_sql)
  })
...
- Uses DuckLake's `ducklake_list_files()` function (the documented API)
