...
This catches problems like `table = "../../../etc/passwd"` before they
cause harm.
...
- Lazy tables use
  ...
  to extract SQL, then `CREATE TABLE AS SELECT`
- `col_types` only applies to data.frame input (lazy tables get types
  from query)
- Supports hive partitioning, bucket partitioning, clustering, and data
  inlining
...
Documentation metadata is stored using DuckLake’s **native COMMENT ON**
with JSON for structured data. This approach:
...
Lineage is stored in the table’s native COMMENT as JSON alongside other
metadata:
...
Views and macros are stored in the DuckLake catalog and support time
travel:
...
- Tree view is rendered dynamically based on available schemas and
  tables
...
7.  Where is documentation metadata stored? (Hint: native COMMENT ON
    with JSON)
